...
def _handle_list_item(item: dict, builder: _PageBuilder) -> None:
    ...
    sentences = [
        restore_merged_hyphens(wrap_bare_greek_letters(wrap_bare_letter_equals_expressions(s.strip())))
        for s in raw_items
        if s.strip()
    ]
    if sentences:
...
def _handle_image_or_table_item(item: dict, item_type: str, images_base, builder: _PageBuilder) -> None:
    ...
    captions = [
        restore_merged_hyphens(wrap_bare_greek_letters(wrap_bare_letter_equals_expressions(c.strip())))
...
def _handle_text_item(item: dict, builder: _PageBuilder, unnumbered_heading_seq: list[int]) -> None:
    ...
    Args:
        unnumbered_heading_seq: 文書全体で共有する、番号無し見出し用の
            ...
            の採番に使う。
    """
    text = restore_merged_hyphens(
        wrap_bare_greek_letters(wrap_bare_letter_equals_expressions(item.get("text", "").strip()))
    )
    if not text:
        return
